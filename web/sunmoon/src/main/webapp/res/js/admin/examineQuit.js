layui.use([ 'table', 'jquery', 'form', 'element'], function() {
	...
	function searchAdd(key) {
		...
		table.reload('post', {
			url : 'Audit/getQuitBySE?key=' + key + '&filter=' + Array.from(filter)+"&department_id="+checkDep,
		});
	}
	// 搜索工具 end
	...
	table.on('tool(post)', function(obj) {
		var data = obj.data;
		 if (obj.event === 'edit') {
			...
			$('#quit_department').html(data.quit_department);
			$('#quit_post').html(data.quit_post);
			$('#quit_time').html(data.quit_time);
			$('#quit_msg').html(data.quit_msg);
