layui.use([ 'table', 'jquery', 'form', 'element'], function() {
	...
	function searchAdd(key) {
		...
		table.reload('post', {
			url : 'Audit/getLeaveBySE?key=' + key + '&filter=' + Array.from(filter)+"&department_id="+checkDep,
		});
	}
	// 搜索工具 end
	...
	table.on('tool(post)', function(obj) {
		var data = obj.data;
		 if (obj.event === 'edit') {
			...
			$('#leave_type').html(data.leave_type);
			$('#leave_msg').html(data.leave_msg);
