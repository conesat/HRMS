layui.use([ 'table', 'jquery', 'form', 'element'], function() {
	...
	function searchAdd(key) {
		...
		table.reload('post', {
			url : 'Audit/getTransferBySE?key=' + key + '&filter=' + Array.from(filter)+"&department_id="+checkDep,
		});
	}
	// 搜索工具 end
	...
	table.on('tool(post)', function(obj) {
		var data = obj.data;
		 if (obj.event === 'edit') {
			...
			$('#transfer_position_name_old').html(data.transfer_position_name_old);
			$('#transfer_department_name_old').html(data.transfer_department_name_old);
			$('#transfer_position_name_new').html(data.transfer_position_name_new);
			$('#transfer_department_name_new').html(data.transfer_department_name_new);
			$('#transfer_msg').html(data.transfer_msg);
