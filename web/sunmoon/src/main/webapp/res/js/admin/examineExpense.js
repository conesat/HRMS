layui.use([ 'table', 'jquery', 'form', 'element'], function() {
	...
	function searchAdd(key) {
		...
		table.reload('post', {
			url : 'Audit/getExpenseBySE?key=' + key + '&filter=' + Array.from(filter)+"&department_id="+checkDep,
		});
	}
	// 搜索工具 end
	...
	table.on('tool(post)', function(obj) {
		var data = obj.data;
		 if (obj.event === 'edit') {
			...
			$('#expense_post').html(data.expense_post);
			$('#expense_department').html(data.expense_department);
			$('#expense_type').html(data.expense_type);
			$('#expense_total_price').html(data.expense_total_price);
			$('#expense_actual_price').html(data.expense_actual_price);
			$('#expense_msg').html(data.expense_msg);			
