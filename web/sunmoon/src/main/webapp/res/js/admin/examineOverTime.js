layui.use([ 'table', 'jquery', 'form', 'element'], function() {
	...
	function searchAdd(key) {
		...
		table.reload('post', {
			url : 'Audit/getOverTimeBySE?key=' + key + '&filter=' + Array.from(filter)+"&department_id="+checkDep,
		});
	}
	// 搜索工具 end
	...
	table.on('tool(post)', function(obj) {
		var data = obj.data;
		 if (obj.event === 'edit') {
			...
			$('#overtime_department').html(data.overtime_department);
			$('#overtime_post').html(data.overtime_post);
			$('#overtime_start_time').html(data.overtime_start_time);
			$('#overtime_end_time').html(data.overtime_end_time);
			$('#overtime_duration').html(data.overtime_duration);
			$('#overtime_msg').html(data.overtime_msg);			
